program nbody
	...
	double precision :: vel_temp, current_time, uranus_distance, AU, ran, theta, phi, vector_length
	! vectors in format (x1,y1,z1), (x2,y2,z2)
	double precision, DIMENSION(3, 8) :: pos,v,a,a2
	double precision, DIMENSION(8) :: m
	double precision, DIMENSION(3) :: r,com,cov
	uranus_distance = 0.0 ! distance from uranus to the sun (AU)
	AU = 1.50e11 ! astronomical unit (m)
	counter = 0 ! used for printing
	t = 0.0 ! the time
	year = 3.15570e7 !number of seconds in a year
	dt = 8640 ! time interval (s)
	...
	m(8) = 0.5*m(1) ! mass of second star
	...
	v(1,8) = -30.0e3 ! second star velocity
	...
	do while (t < 2000*year)
		...
		! update position, it's now at i+1
		do i = 1, n, 1
			pos(1:3,i) = pos(1:3,i) + v(1:3,i)*dt + 0.5*a(1:3,i)*dt*dt ! move the objects
		end do

		! find acceleration i+1
		a2 = 0.0
		do i = 1,n
			do j = 1,n
				if (j == i) CYCLE
		...
		! update velocity, it's now at i+1
		do i = 1, n, 1
		...
		do i = 1, n, 1
			...
			if ( counter >= 100000 ) then
				...
				print *, abs(((e_t-e_0)/e_0)*100),",", current_time,",", uranus_distance
				counter = 0
			end if
		end do
		t = t + dt ! step time
